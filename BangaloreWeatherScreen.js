...
const BangaloreWeatherScreen = () => {
  ...
  useEffect(() => {
    const fetchWeather = async () => {
      ...
      setError(null);
      try {
        const response = await axios.get(`https://api.weatherapi.com/v1/current.json?key=${WEATHER_API_KEY}&q=Bangalore`);
    ...
    <View style={styles.container}>
      {loading ? (
        ...
        <View style={styles.weatherContainer}>
          <Text style={styles.title}>Weather in Bangalore</Text>
          <Image
...
export default BangaloreWeatherScreen;
